...
from ...d3rlpy.containers import FIFOQueue, SegmentTree
...
from d3rlpy.envs import BatchEnv
...
class _PER(metaclass=ABCMeta):

    _transitions: SegmentTree[Transition]
    _observation_shape: Sequence[int]
    _action_size: int
    _create_mask: bool
    _mask_size: int
    ...
    def __init__(
        ...
        self._transitions = SegmentTree(maxlen)
    ...
    def append_episode(self, episode: Episode) -> None:
        ...
        assert episode.get_action_size() == self._action_size
        for transition in episode.transitions:
            self._transitions.append(transition, self._transitions.max)
            # add mask if necessary
            if self._create_mask and transition.mask is None:
                transition.mask = np.random.randint(2, size=self._mask_size)
    ...
    @property
    def transitions(self) -> SegmentTree[Transition]:
        return self._transitions
...
class PriorityReplayBuffer(_PER, Buffer):
    ...
    def __init__(
        ...
        episodes: Optional[List[Episode]] = None,
        create_mask: bool = False,
        mask_size: int = 1,  # ensemble size for mask
    ):
        super().__init__(maxlen, env, episodes, create_mask, mask_size)
        self._prev_observation = None
        self._prev_action = None
    ...
    def append(
        ...
        reward: float,
        terminal: float,
        clip_episode: Optional[bool] = None,  # flag to clip the current episode
    ) -> None:
        # if None, use terminal
        if clip_episode is None:
            clip_episode = bool(terminal)
    ...
    def _get_transitions(self, idxs):
        transitions = self._transitions.get(data_index=idxs)
        return transitions
    ...
    def _get_samples_from_segments(self, batch_size, p_total):
        ...
        probs, idxs, tree_idxs = self._transitions.find(samples)
    ...
    def sample(self, batch_size, n_steps=1, n_frames=1, gamma=0.99):
    ...
    def update_priorities(self, idxs, priorities):
        self._transitions.update(idxs, priorities)
